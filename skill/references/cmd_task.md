...
Resume the coding agent — using the saved agent handle — with the Commit Prompt template below. The coding agent commits all changes, marks the task complete, and returns the commit message. On the UI review's quick-fix route, the quick-fix agent commits instead — see [references/shared/ui_review.md](shared/ui_review.md).
