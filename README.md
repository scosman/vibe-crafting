...
The skill lives in [`skill/`](skill). Install it and you get ten `/spec` commands (plus a bare `/spec` router) in the agent of your choice.

## Why

Two numbers drive the whole design:

- **~90% of the decisions that matter happen during planning.**
- **~90% of the tokens get spent during coding.**

So put your attention where the decisions are. You and an interactive agent go broad-to-specific before a line of code is written: project overview → functional spec → architecture → component designs → implementation plan → per-phase plans. By the time a coding agent starts a phase, it isn't winging it — it's executing a self-contained plan against quality gates it can't exit without passing (build, lint, format, tests, code review).

That's the difference from vibe coding. I don't write the code and I don't read every line as it lands, but the agent doesn't get to improvise either. The result has consistently been better than what I'd write by hand — not because I couldn't, but because I'd never have bothered with 288 tests and a full UI test suite.
...
The skill is a standard agent skill: a `SKILL.md` plus a `references/` folder. The **directory name is the command name**, so the contents of `skill/` need to land in a directory called `spec`.
...
Shortest real path from nothing to shipped:
